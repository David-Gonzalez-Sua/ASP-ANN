...
save_values = args.v  # If true, saves neuron values in the output file, and saves to snapshots folder.
...
if toks_next[0].startswith("OPT") or toks_next[0].startswith("SAT"):
    ...
    for t in toks:
        if t.startswith("neuron"):
            ...
            index = int(param[2])
            if scaled_integer:
                excitation = f'{int(param[3])}'
            else:
                excitation = f'{str(param[3])}'

            facts.append(f"excitation({n_type}, {layer}, {index}, {excitation}).")
    ...
    if save_values:
        with open(f"Adaptive_Network/Clingo_ANN/model_snapshots/{file_name}", "w") as f:
            f.write("\n".join(facts))
    else:
